...
def plausibility_check_experiment_results(experiment_results: ExperimentResults):
    ...
    for agent_id in route_dag_constraints_delta_afer_malfunction:
        ...
        node_set_delta_after_malfunction = set(experiment_results.problem_delta_after_malfunction.topo_dict[agent_id].nodes)
        if not node_set_full_after_malfunction.issuperset(node_set_delta_after_malfunction):
            assert node_set_full_after_malfunction.issuperset(node_set_delta_after_malfunction), \
                f"{agent_id}: not all delta nodes are also in full: only delta " \
                f"{node_set_delta_after_malfunction.difference(node_set_full_after_malfunction)}, " \
