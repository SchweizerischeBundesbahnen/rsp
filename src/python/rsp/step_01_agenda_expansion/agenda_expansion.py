...
def expand_range_to_parameter_set(parameter_ranges: List[Tuple[int, int, int]]) -> List[List[int]]:
    """Expand parameter ranges.

    Parameters
    ----------
    parameter_ranges
        Ranges are of the form `start` (incl), `stop` (excl), `step`.  If `step == 1`, then stop is ignored.
