...
class _c_value(Union):
    _fields_ = [("integer", c_int), ("double", c_double), ("symbol", c_uint64)]


class _c_variant(Structure):
    _fields_ = [("type", c_int), ("value", _c_value)]
...
class Theory:
    def __init__(self, prefix, lib):
        ...
        self.__create_propagator = self.__fun(prefix, "create", c_bool, [POINTER(c_void_p)])

        # bool destroy_propagator(propagator_t *propagator);
        self.__destroy_propagator = self.__fun(prefix, "destroy", c_bool, [c_void_p])

        # bool register_propagator(propagator_t *propagator, clingo_control_t* control);
        self.__register_propagator = self.__fun(prefix, "register", c_bool, [c_void_p, c_void_p])
