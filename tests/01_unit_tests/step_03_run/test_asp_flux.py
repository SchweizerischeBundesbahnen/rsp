...
def test_asp_helper():
    ...
    expected = set(["dl((t1,(t1,4)),1)", "dl((t1,(t1,1)),0)"])
...
def test_simple_rail_asp_one_agent():
    ...
    tc = create_schedule_problem_description_from_instructure(
        infrastructure=create_infrastructure_from_rail_env(env, k=k), number_of_shortest_paths_per_agent_schedule=k
    )
    ...
    expected = set(["dl((t0,((3,1),1)),0)", "dl((t0,((3,2),1)),1)", "dl((t0,((3,3),1)),2)", "dl((t0,((3,4),1)),3)", "dl((t0,((3,5),1)),4)"])

    assert actual_answer_set.issuperset(expected), "expected={} should be subset of actual={}".format(expected, actual_answer_set)
...
def test_minimize_sum_of_running_times_scheduling():
    ...
    expected = set(["dl((t1,1),2)", "dl((t1,2),4)", "dl((t1,3),6)", "dl((t2,4),0)", "dl((t2,2),2)", "dl((t2,3),4)"])
