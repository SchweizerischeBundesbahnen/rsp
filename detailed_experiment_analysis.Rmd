...
```{python}
changed_agents_traces = plot_many_time_resource_diagrams(experiment_data_frame=experiment_data,experiment_id = experiment_of_interest, with_diff=True)
```
...
```{python}
schedule_times, schedule_ressources =plot_schedule_metrics(experiment_data_frame=experiment_data,experiment_id = experiment_of_interest)
```
...
```{python}
plot_time_window_resource_trajectories(experiment_result=single_exp_results)
```
...
(distance_matrix_full, distance_matrix_malfunction, distance_matrix_diff) = plot_encounter_graphs_for_experiment_result(experiment_result=single_exp_results,highlighted_nodes=changed_agents,pos=fixed_positions,metric_function=symmetric_temporal_distance_between_trains)
```

```{python}
fixed_positions = {agent_of_interest:(-100,-100)}
plot_encounter_graphs_for_experiment_result(experiment_result=single_exp_results,highlighted_nodes=changed_agents,metric_function=symmetric_temporal_distance_between_trains)
```

```{python}
fixed_positions = {agent_of_interest:(0,0)}
plot_encounter_graphs_for_experiment_result(experiment_result=single_exp_results,highlighted_nodes=changed_agents, pos=fixed_positions)
```
