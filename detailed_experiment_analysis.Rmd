...
```{python}
debug_info, distance_matrix, weights_matrix = disturbance_propagation_graph_visualization(experiment_result=single_exp_results)
```

```{python}
from rsp.hypothesis_two_encounter_graph import _debug_directed
```

```{python}
_debug_directed((0, 24), debug_info, distance_matrix, weights_matrix)
```
