...
A schedule is a function $S: \mathcal{A}\times \mathbb{N} \to \mathcal{V} \cup \{ \bot \}$, where $\bot$ denotes the fact that a train is not in the grid; we only consider non-negative discrete time steps $\mathbf{N}$.
Before defining valid schedules, we need some notation.
Define the nodes visited by a train $a$ as
\begin{equation*}
\mathcal{V}(S,a) = \Set*{ S(a,t): t\in\mathbf{N}}
\end{equation*}
and
\begin{equation*}
...
\end{equation*}
for $v \in \mathcal{V}(S,a)$. Notice that the exit time corresponds to the entry time in the next cell (which is one time step after the last time step on the edge starting at $v$) and should not be confused with the release time of the cell, which comes $r$ time steps after the exit time.
\begin{mdframed}
{\bf TODO Christian} something not koscher: distinguish last node without +1? speed rule at target??? speed rule on edges instead of Sexit on vertex!!!\\
{\bf TODO Christian} should we harmonize with general train scheduling problem, were have only entry times?
\end{mdframed}
...
\item[service intention] the first and the last position of a train run is the required source and target, i.e. for all trains $a \in \mathcal{A}$ there is $t_{source}$ and $t_{target}$ such that $S(a,t_{source})=\sigma(a)$, $S(a,t_{target})\in\tau(a)$, $S(a,t)=\bot$ for $t<t_{source}$ and $t>t_{target}$;
...
\item[speed] each agent has a constant \emph{minimum running time}, we require $S_{exit}(a,t)-S_{entry}(a,t)\geq {v(a)}^{-1}$, i.e. .
...
The time windows given by $e$ and $l$ can be computed by Algorithm~\ref{algo:propagate_earliest} and \ref{algo:propagate_latest}, respectively. They propagate the minimum running forward from an initial set of earliest and backwards from an initial set of latest constraints.
...
A \emph{solution to a train scheduling problem} $(N, \mathcal{A}, C, F)$ is a pair $(P, A)$ consisting of
\begin{enumerate}
...
It is easy to see that any schedule $S_{entry}$ is an assignment to the general train scheduling problem after transformation, hence the given transformation is indeed an embedding, carrying solutions to solutions.



\begin{mdframed}
{\bf TODO Christian} should we introduce removal of edges not reached in time here?\\
{\bf TODO Christian} should we introduce notation for sources and sinks?\\
...
\begin{mdframed}
{\bf TODO Christian} does it respect the malfunction? $v_1$ yes, but not $v_2$???
\end{mdframed}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
	   ...
	   \For{$v \in S_1$}
	    ...
	    \EndFor
	    \State $e,l,(S_1,L_1) \leftarrow propagate(e,l,(S_1,L_1),\Delta_A, \Delta_A, \Delta_P, mrt, U, c)$
	\end{algorithmic}
\end{algorithm}
